    ...
    do {
        while((move_type = move()) != 0) {
            if(move_type == 1) { // LEAF CANCEL
                ...
                /* FIX FROM HERE */
                extend_history(rulecount);
                for(int i = rulecount; i < g.rules.size(); i++) {
        ...
        if (final_config() == true) {
            ...
            BT_ITEM most_recent_history = history.back();
            history.pop_back();
            pda = most_recent_history.pda;
            progress = most_recent_history.progress;
            rulecount = most_recent_history.rulecount + 1;
...
void TDPARSE_BT::extend_history(unsigned int rulecount) {
    ...
    if(later) {
        ...
        BT_ITEM tdParse;
        tdParse.pda = pda;
        tdParse.rulecount = rulecount;
        tdParse.progress = progress;
        history.push_back(tdParse);
